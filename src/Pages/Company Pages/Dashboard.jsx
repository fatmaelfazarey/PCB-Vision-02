                    ...
                    <div className="flex flex-row mt-4 overflow-scroll scrollHedden">
                        <nav className='w-fit min-w-32 mr-4 h-full min-h-[80vh] bg-second dark:bg-second-dark rounded-xl overflow-hidden'>
                            <ul className='flex flex-col justify-between gap-4 w-full'>
                                <li className={`${isActive('statistics') ? 'bg-main w-full text-white' : 'text-gray-600'} p-2 `}><Link to='statistics' >Statistics</Link></li>
                                <li className={`${isActive('add-employee') ? 'bg-main w-full text-white' : 'text-gray-600'} p-2 `}><Link to='add-employee' >Add Employee</Link></li>
                                <li className={`${isActive('employees') ? 'bg-main w-ful text-white' : 'text-gray-600'} p-2 `}><Link to='employees' >All Employees</Link></li>
                                <li className={`${isActive(`company-profile/${employeeId}`) ? 'bg-main w-ful text-white' : 'text-gray-600'} p-2 `}><Link to={`company-profile/${employeeId}`} >Profiles</Link></li>
                            </ul>
                        </nav>
