...
const LoginComponent = (props) => {
    ...
    }, [formData, rememberMe, props.isCompanyLogin, validate, LoginUser, LoginAsCompany, navigate, setEmployee, setEmployeeId, setRole]);
    ...
    return (
        <div className="relative bg-second dark:bg-second-dark flex items-center h-screen">
            <div className={`h-screen absolute top-0 left-0 md:relative bg-white dark:bg-black shadow-lg sm:w-130 w-[90%] flex flex-col justify-center items-center p-5 ${language === 'ar' ? 'rounded-l-4xl' : 'rounded-r-4xl'}`}>
                    ...
                    <div className='w-full flex justify-between items-center mb-4'>
                        <label className="flex items-center gap-1 cursor-pointer">
                            <input
                                ...
                                className="hidden"
                            />
                            <div className={`w-5 h-5 border-2 border-main rounded-[4px] flex justify-center items-center ${rememberMe ? 'bg-main' : 'bg-transparent'}`}>
                                {rememberMe && (
                                    <svg width="14px" height="14px" viewBox="0 0 36 36" aria-hidden="true" role="img" preserveAspectRatio="xMidYMid meet">
                                        <path fill="white" d="M34.459 1.375a2.999 2.999 0 0 0-4.149.884L13.5 28.17l-8.198-7.58a2.999 2.999 0 1 0-4.073 4.405l10.764 9.952s.309.266.452.359a2.999 2.999 0 0 0 4.15-.884L35.343 5.524a2.999 2.999 0 0 0-.884-4.149z"></path>
                                    </svg>
                                )}
                            </div>
                ...
                <div className='text-center text-sub-text text-base'>
                    <p>
                        {t('Login ')}
                        <Link to={props.isCompanyLogin ? '/login' : '/login-company'}>
                            <u className='text-blue-600'>{props.isCompanyLogin ? t('personal account') : t('company account')}</u>
                        </Link>
                    </p>
                    <p>
                        {t("You don't have an account? ")}
                        <Link to='/sign'>
                            <u className='text-blue-600'>{t('create account')}</u>
                        </Link>
                    </p>
                </div>
            </div>
